...
Run from the repository root. The generated ZIP has `manifest.json`, both icons, and `skills/` at the archive root. The Microsoft 365 manifest uses schema version 1.28 and intentionally contains no unsupported root properties.

```bash
python scripts/validate_repo.py
python scripts/build_plugin.py it-helpdesk-cowork
```
