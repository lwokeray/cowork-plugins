...
1. 從 GitHub Release 下載 `pm-cowork-<version>.zip`，或依下方指令自行建置。
2. 在 Microsoft 365 管理中心上傳自訂 app package，或於 Copilot Cowork 的 Sources & Skills 管理介面上傳 plugin。
3. 由管理員依租戶政策核准並指派使用者。
4. 在 Cowork 中確認 16 個 Skills 可被發現，再以 `evals/skill-evals.json` 的 prompts 做 smoke test。

部署 ZIP 根目錄只包含 `manifest.json`、`color.png`、`outline.png` 與 `skills/`。`prompts/`、`evals/`、`docs/` 只供來源維護與驗證，不放入上傳套件。
...
python scripts/build_plugin.py pm-cowork
```
