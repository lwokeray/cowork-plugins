...
對每位與會者只使用已驗證資訊描述其職務、實際參與議題、過往陳述、已承諾行動及需要向其確認的事項。無法確認角色時寫 `角色未知`，不從職稱推斷其為 Sponsor、Decision Maker 或 Blocker。

將問題依對象安排：

- Executive：聚焦 Business outcome、變更理由、決策與風險。
- Business／Operations：聚焦目前流程、影響、採用與衡量方式。
- Technical：聚焦環境、整合、限制、Security、運作與驗證。
- Procurement／Legal：聚焦流程、必要文件、核准順序與時程條件。
- Partner：聚焦責任分工、客戶成果與共同下一步。

此分類只用於調整問題，不代表已確認的決策權。

### 階段六：設計會議流程

會議設計必須包含：

1. **Opening**：用一段話確認會議目的與時間。
2. **Context check**：確認上次決議或近期變化是否仍有效。
3. **核心討論**：最多三個直接支援 Meeting outcome 的議題。
4. **Discovery／Decision**：對最重要未知進行驗證或取得決策。
5. **Close**：重述已確認事項、Owner、日期及下一個 Checkpoint。

只有使用者要求時才加入 Demo、Presentation 或 Proposal review；明確說明其目的、使用時間與要取得的反饋。不要用產品簡報占滿整場會議。

## 完整 Meeting Brief 結構

1. 會議識別、時間、時區、參與者、Organizer、位置／Channel。
2. 一句 Meeting outcome 與建議結束狀態。
3. 為何現在開會：近期客戶事件、承諾或決策背景。
4. 已確認客戶成果與目前狀態。
5. 與會者角色證據及每人的關鍵確認事項。
6. 上次以來的重要變化、雙方承諾及完成狀態。
7. 未解 objection、風險、依賴及不應在會議中宣稱的內容。
8. 時間分配合理的 Agenda。
9. 依優先順序排列的 Discovery／Decision questions。
10. 建議下一步、應確認的 Owner 與日期。
11. 來源、觀察日期、未知與無法存取項目。

## Discovery 問題品質規則

- 每題只驗證一個未知。
- 問題使用中性語言，不預設客戶同意問題、方案或時程。
- 先問目前做法與影響，再問理想結果與決策條件。
- 不詢問內部來源已經能可靠回答的基本問題。
- 不用連續封閉問題逼出預期答案。
- 問題須能改變下一步、Proposal、Stakeholder path 或 Qualification。

## 停止條件

- 無法唯一解析會議時，只列候選，不混合內容。
- 找不到會議內容或 Account evidence 時，產出最小 Brief 並明示證據限制。
- 私人行程只可用於衝突，不讀取或揭露內容。
- 使用者要求寄送 Agenda、修改 Invite 或建立後續行動時，先完成 Brief，再轉交相應 Skill。
...
- 交付能在會議中直接使用的 Brief，不說明 Agent 如何搜尋資料。
- 使用與受眾相符的商務語言；不顯示 Work IQ、tool、path、schema 或 payload。
- 不輸出隱藏思考；只提供證據、會議策略、未知與問題設計。
- Meeting outcome、參與者角色證據、近期變化、Agenda、問題與 Close 均完整。
- 所有時間已標示時區，Private event 內容未被揭露。
