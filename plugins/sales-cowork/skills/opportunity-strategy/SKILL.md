...
## Bid／No-bid 與 Blue Review

當商機包含正式 RFP、RFQ 或需要大量跨部門投入的 Proposal 時，建立投標決策材料，而不是直接假設應該參與。

Bid／No-bid Brief 必須包含客戶成果與評選方式、策略適配性、資格與 Mandatory requirement、買方接觸與決策證據、競爭或替代方案、Solution／Delivery／Commercial 可行性、資源與期限、重大風險、所需投資、Evidence gaps 及具名 Decision owner。結論只能是 `Recommend bid`、`Recommend no-bid` 或 `Decision blocked`，並列出依據與信心；最終決策保留給授權人員。

Blue Review 在完整提案撰寫前檢查：

- 是否理解客戶真正要改變的 Outcome，而不是只看到產品需求。
- Evaluation criteria、Decision path、Stakeholder 與競爭態勢是否有證據。
- Win themes 是否連結客戶證據與可證明的差異，而非內部口號。
- 是否存在會使投標不可行的資格、時程、責任或商務條件。
- 哪些內容仍是 Seller hypothesis，以及如何向客戶或 Owner 驗證。

Blue Review 不建立完整答案，也不核准 Solution、Price 或 Submission。通過後才將 Pursuit Brief、Win strategy、Requirement package 與 Evidence gaps 交給 `proposal-preparation`。
