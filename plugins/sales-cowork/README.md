...
版本 3.1 採用單檔平鋪 Monolithic Prompt Packing，並吸收原 Proposal Plugin 的必要能力。每個 Skill 都包含完整的證據判斷、Sales 工作流程、決策規則、產出結構、停止條件、例外處理、核准與驗證要求；Agent 不需讀取外部 reference 才能完成該 Skill 的核心工作。
...
| Proposal | `proposal-quality-review` | 執行 Blue／Pink／Red／Green／Gold Review、引證檢查與送件就緒判定。 |
...
部署 ZIP 根目錄只包含 `manifest.json`、`color.png`、`outline.png` 與 `skills/`。README、docs、prompts 與 evals 是 Repository materials，不放入部署 ZIP。
