...
description: 建立或更新可稽核的 Operating model、Three-statement model、Debt、Capex、Working capital、Scenario 與 Sensitivity schedules，並維持 Formula consistency、Source traceability 與 Model checks。 適用於 Integrated financial modeling、延伸 Forecast periods、更新 Actuals／Drivers、建立 Cash／Debt／Equity roll-forwards；不適用於單純 Budget collection、只做 DCF、硬編輸出、破壞既有 Workbook 或未驗證來源的精確模型。
metadata:
  author: lwokeray
  version: 1.0.1
license: MIT
