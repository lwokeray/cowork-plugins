...
版本 1.0.0 使用單檔平鋪 Monolithic Prompt Packing。每個 `skills/<skill-name>/SKILL.md` 都能獨立完成核心工作，內含適用／排除條件、工作流程、來源與證據規則、計算與Tie-out、產出契約、停止條件、核准邊界、例外處理、Work IQ工具規則、範例與常見問題，不依賴外部Reference才可運作。使用者收到完整Finance artifact，不會收到Agent思考過程或製作摘要。
...
```bash
mkdir -p dist
(cd finance-cowork && zip -X -qr -FS ../dist/finance-cowork.zip manifest.json color.png outline.png skills)
python3 finance-cowork/tests/validate_package.py finance-cowork dist/finance-cowork.zip
```

部署ZIP根目錄只包含`manifest.json`、`color.png`、`outline.png`與`skills/`。README、docs與tests是Repository materials，不放入部署ZIP。
