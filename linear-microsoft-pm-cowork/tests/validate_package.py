#!/usr/bin/env python3
"""PM Cowork skills-only package 與 Prompt Card 靜態驗證。"""
...
def validate_prompts(package: Path, expected_count: int) -> None:
    ...
    for index, row in enumerate(rows, start=2):
        ...
        if row["Department"] != "產品管理":
            fail(f"Prompt Card 第 {index} 列 Department 必須為產品管理")
...
def validate_zip(package: Path, archive_path: Path, expected_skills: set[str]) -> None:
    ...
    with zipfile.ZipFile(archive_path) as archive:
        ...
        if any(name.startswith("linear-microsoft-pm-cowork/") for name in names):
            fail("部署 ZIP 不得包含外層 linear-microsoft-pm-cowork 目錄")
...
def main() -> None:
    ...
    package = Path(sys.argv[1] if len(sys.argv) >= 2 else "linear-microsoft-pm-cowork")
    ...
    for entry in skills:
        ...
        skill_files = {path.name for path in skill_dir.iterdir()}
        if skill_files != {"SKILL.md"}:
            fail(f"{skill_dir} 必須採單檔 Monolithic Prompt Packing，不得包含其他檔案")
