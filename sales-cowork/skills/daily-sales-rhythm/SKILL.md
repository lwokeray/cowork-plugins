...
description: >-
  Prioritizes up to three evidence-backed sales actions for today from Work IQ and available Sales context.
...
Return the smallest useful set of next actions for the current day. Use Work IQ, Daily Briefing, Calendar Management, Enterprise Search, and available Dynamics 365 Sales context to retrieve permission-accessible meetings, email and Teams commitments, documents, people, and account signals.
...
2. Retrieve permission-accessible same-day meetings, customer commitments, due work, and time-sensitive account signals through Work IQ and available plugins. If a required source or plugin is unavailable, label only that field `unavailable` and continue with the remaining evidence.
