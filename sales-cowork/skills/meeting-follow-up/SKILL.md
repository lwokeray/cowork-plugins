...
Create three distinct review artifacts from one completed meeting: an internal note, an Outlook-ready customer email draft, and task proposals. Use only a recap, transcript, chat export, notes, or related evidence supplied to the session. The skill remains draft-only because this skills-only package has no read or write connector.

## Guardrails

- Resolve one completed meeting before producing any artifact. Stop when the meeting or evidence is ambiguous.
- Use supplied Teams recap or transcript, meeting metadata, and related messages as evidence. If none is supplied or surfaced by Cowork, return `Source unavailable — user input needed`. Label each item as `stated`, `inferred`, or `unresolved`.
...
1. Identify one completed customer meeting from supplied recap, transcript, chat, or follow-up context.
