# Copilot Cowork IT Operations Plugin
...
|---|---|
| `copilot-cowork-it` | Infrastructure and platform operations, identity, endpoints, M365, cloud, network, security, recovery, assets, changes, vendors, automation, and operational governance |
...
Create the tenant-uploadable ZIP with `manifest.json`, both icons, and `skills/` at the archive root:

```bash
mkdir -p dist
(cd copilot-cowork-it && zip -qr ../dist/copilot-cowork-it.zip manifest.json color.png outline.png skills)
unzip -t dist/copilot-cowork-it.zip
```
