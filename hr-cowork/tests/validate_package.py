#!/usr/bin/env python3
"""HR Cowork skills-only package 與 Prompt Card 靜態驗證。"""
...
WORK_ONLY_PROMPTS = {
    "協調面試時程", "建立 Onboarding 計畫", "處理人事資料異動",
    "建立 ER Case Brief", "建立 Offboarding 計畫",
}
...
def validate_prompts(package: Path, expected_count: int) -> None:
    ...
    for index, row in enumerate(rows, start=2):
        ...
        if row["Department"] != "人力資源":
            fail(f"Prompt Card 第 {index} 列 Department 必須為人力資源")
...
def validate_zip(package: Path, archive_path: Path, expected_skills: set[str]) -> None:
    ...
    with zipfile.ZipFile(archive_path) as archive:
        ...
        if any(name.startswith("hr-cowork/") for name in names):
            fail("部署 ZIP 不得包含外層 hr-cowork 目錄")
...
def main() -> None:
    ...
    package = Path(sys.argv[1] if len(sys.argv) >= 2 else "hr-cowork")
